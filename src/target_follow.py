...
# when turning to goal, this is the tolerance +/- in radians
...
# Camera Parameters
# (degrees) for D435, 68 is measured, diff from spec: https://www.intel.com/content/www/us/en/support/articles/000030385/emerging-technologies/intel-realsense-technology.html
HORIZONTAL_FOV = 68
VERTICAL_FOV = 57
...
class SmartCart:
    def __init__(self):
        self.goal_pose = Pose(Point(0.0, 0.0, 0.0),
                              Quaternion(0.0, 0.0, 0.0, 1.0))
        self.current_pose = Pose(Point(0.0, 0.0, 0.0),
                                 Quaternion(0.0, 0.0, 0.0, 1.0))
        ...
        # robot's current angle in radians relative to odom frame's positive x-axis; CCW is positive
        self.currentYaw = 0.0
        ...
        self.LED_pub = rospy.Publisher(
            'LEDsignal', Bool, queue_size=QUEUE_SIZE)
        self.LED_rate = rospy.Rate(LED_PUBLISH_RATE)

        self.target_pose_pub = rospy.Publisher(
            'target_pose', PoseStamped, queue_size=QUEUE_SIZE)
        ...
        self.odom_sub = rospy.Subscriber(
            '/robot_pose_ekf/odom_combined', PoseWithCovarianceStamped, self.odomProcess)

        self.target_pos_sub = rospy.Subscriber(
            'target_position', Int32MultiArray, self.target_pos_process)
        self.target_dist_sub = rospy.Subscriber(
            'target_distance', Float32, self.target_dist_process)
    ...
    def odomProcess(self, odomData):
        ...
        self.currentYaw = euler_from_quaternion(
            [odomData.pose.pose.orientation.x, odomData.pose.pose.orientation.y, odomData.pose.pose.orientation.z, odomData.pose.pose.orientation.w])[2]
    ...
    def get_deltaYaw(self):
        ...
        self.deltaYaw_unfiltered = atan2(self.goal_pose.position.y - self.current_pose.position.y,
                                         self.goal_pose.position.x - self.current_pose.position.x) - self.currentYaw
    ...
    def locate_next_waypoint(self, verbose=True):
        if self.target_dist < 0.01 or self.target_dist > 10 or np.isnan(self.target_dist):
            ...
            # Horizontal pixels from center of camera
            x_cam = self.target_pos[0] - HORIZONTAL_RESOLUTION / 2
            # Vertical pixels from center of camera
            y_cam = self.target_pos[1] - VERTICAL_RESOLUTION / 2

            # New code assuming the camera corrects for depth to be x
            # Construct right triangle with sides x_px, y_px, d_px in robot frame
            x_px = (HORIZONTAL_RESOLUTION/2) / \
                np.tan(np.deg2rad(HORIZONTAL_FOV)/2)
            y_px = x_cam
...
############################################## Goal Setting/Getting Functions ########################################################
    ...
    def getNextGoal(self):
        ...
        if self.euclidean_distance() > DISTANCE_TOLERANCE:
            ...
            print("Goal position you input is too close to the current position, please change your waypoints so they are greater than ",
                  DISTANCE_TOLERANCE, "metres from each other")
            self.state = STATE_AT_GOAL
        return
...
if __name__ == "__main__":
    try:
        ...
        while not rospy.is_shutdown():  # run infinite loop
            # Constantly run to add next waypoint to queue
            cart.get_next_waypoint(verbose=False)
